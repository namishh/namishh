...
enum { SchemeNorm, SchemeSel, SchemeHid, SchemeButton }; /* color schemes */
enum {
  NetSupported,
  NetWMName,
  NetWMState,
...
int drawstatusbar(Monitor *m, int bh, char *stext) {
  ...
  w += 2 + horizpadbar; /* 1px padding on both sides */
  ret = m->ww - w;
    ...
    if (text[i] == '^' && !isCode) {
      ...
      while (text[++i] != '^') {
        if (text[i] == 'c') {
          ...
          drw_clr_create(drw, &drw->scheme[ColBg], accent);
        } else if (text[i] == 'A') {
          drw_clr_create(drw, &drw->scheme[ColFg], accent);
        } else if (text[i] == 'q') {
          drw_clr_create(drw, &drw->scheme[ColBg], normbgcolor);
        } else if (text[i] == 'Q') {
          drw_clr_create(drw, &drw->scheme[ColFg], normbgcolor);
        } else if (text[i] == 'w') {
          drw_clr_create(drw, &drw->scheme[ColBg], normfgcolor);
        } else if (text[i] == 'W') {
          drw_clr_create(drw, &drw->scheme[ColFg], normfgcolor);
        } else if (text[i] == 'B') {
          int c = atoi(text + ++i);
  ...
  if (!isCode) {
    w = TEXTW(text) - lrpad;
    drw_text(drw, x, vertpadbar / 2, w, bh - vertpadbar / 2, 0, text, 0);
  }
...
void drawbar(Monitor *m) {
  int x, w, tw = 0, n = 0, scm, stw = 0, blw = 0;
...
void saveSession(void) {
  ...
  for (Client *c = selmon->clients; c != NULL;
       c = c->next) { // get all the clients with their tags and write them to
                      // the file
    fprintf(fw, "%lu %u\n", c->win, c->tags);
  }
  fclose(fw);
...
void restoreSession(void) {
  ...
  char *str = malloc(
      23 *
      sizeof(char)); // allocate enough space for excepted input from text file
  while (fscanf(fr, "%[^\n] ", str) != EOF) { // read file till the end
    ...
    for (Client *c = selmon->clients; c;
         c = c->next) { // add tags to every window by winId
      if (c->win == winId) {
        c->tags = tagsForWin;
        break;
      }
...
void setup(void) {
  ...
  bh = drw->fonts->h + 2 + user_bh + vertpadbar;
  updategeom();
  /* init atoms */
